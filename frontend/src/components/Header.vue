<template>
  <header class="glass-effect shadow-soft border-b border-secondary-200/50 dark:border-secondary-700/50 z-30">
    <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
      ...
      <div class="flex items-center">
        ...
        <nav class="mr-4 hidden sm:flex" aria-label="Breadcrumb">
          <ol class="flex items-center space-x-2 space-x-reverse">
            <li>
              <router-link to="/"
                class="text-secondary-500 dark:text-secondary-400 hover:text-secondary-700 dark:hover:text-secondary-200 text-sm">
                الرئيسية
              </router-link>
            </li>
            <li v-if="currentPageName !== 'الرئيسية'">
              <span class="text-sm font-medium text-secondary-900 dark:text-secondary-100 mr-2"> > {{ currentPageName
                }}</span>
            </li>
          </ol>
        </nav>
      ...
      <div class="flex items-center space-x-4">
        ...
        <div class="hidden md:block relative">
          ...
          <input v-model="searchQuery" type="text" placeholder="البحث السريع..."
          ...
          <div v-if="showNotifications"
            ...
            <div v-else class="divide-y divide-secondary-100">
              <div v-for="notification in notifications.slice(0, 5)" :key="notification.id"
                ...
                :class="{ 'bg-primary-50/50': !notification.read }" @click="markAsRead(notification.id)">
                <div class="flex items-start">
                  <div class="flex-shrink-0 w-2 h-2 rounded-full mt-2" :class="getNotificationColor(notification.type)">
                  </div>
    ...
    <!-- Mobile Search -->
    <div v-if="showMobileSearch" class="md:hidden px-4 pb-4 border-t border-secondary-200">
      <div class="relative">
        <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
          <MagnifyingGlassIcon class="h-4 w-4 text-secondary-400" />
        </div>
